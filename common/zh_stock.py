...
def get_today_latest_price(stock_symbol_list) -> dict:
    ...
    for stock_symbol in stock_symbol_list:
        ...
        if stock_today_data is not None:
            latest_price_dict[stock_symbol] = stock_today_data["summary"][cs.LATEST_PRICE]
    
    return latest_price_dict

def get_today_data(stock_symbol):
    
    stock_today_data_file = ZH_STOCKS_DATA_PATH / stock_symbol[0] / stock_symbol / f"{supports.today()}.json"
    if not stock_today_data_file.exists():
        return None
    
    with open(ZH_STOCKS_DATA_PATH / stock_symbol[0] / stock_symbol / f"{supports.today()}.json", 'r') as f:
