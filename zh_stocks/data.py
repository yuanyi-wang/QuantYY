...
REDIS_CLIENT = Client(
    host=supports.APP_CONFIG["redis"]["host"],
    port=supports.APP_CONFIG["redis"]["port"],
    db=0,
    password=supports.APP_CONFIG["redis"]["password"],
    ...
    with ThreadPoolExecutor(max_workers=supports.CPU_COUNT - 1) as thread_pool:
        for index, row in df_stock_zh.iterrows():
            ...
            task = thread_pool.submit(
                _generate_min_price_data, row, today_str, now_str, price_big_change_warn
            )
...
def _generate_min_price_data(row, str_date, str_time, price_big_change_warn):
    ...
    stock_symbol = row["代码"]
    item = {cs.TIME: str_time, "price": supports.j(row["最新价"])}
    summary = {
        cs.DATE: str_date,
